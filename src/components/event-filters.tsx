...
import { Calendar, Filter, MapPin, Music, User, Building2, X } from "lucide-react"
...
export function EventFilters({ filters, onFiltersChange, events = [] }: EventFiltersProps) {
  ...
  const artists = Array.from(new Set(events.map((e) => e.artist).filter((a): a is string => a !== undefined)))
      ...
      <div className="flex items-center gap-2 flex-wrap">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2 max-h-[200px] overflow-y-auto">
                  {venues.map((venue) => (
                    <label key={venue} className="flex items-center gap-2 cursor-pointer hover:bg-muted p-2 rounded-md">
                      <Checkbox checked={filters.venues.includes(venue)} onCheckedChange={() => toggleVenue(venue)} />
        ...
        {filters.venues.map((venue) => (
          <Badge key={venue} variant="secondary" className="gap-1">
            {venue}
            <X className="h-3 w-3 cursor-pointer" onClick={() => toggleVenue(venue)} />
          </Badge>
        ))}
        {filters.artists.map((artist) => (
          <Badge key={artist} variant="secondary" className="gap-1">
            {artist}
            <X className="h-3 w-3 cursor-pointer" onClick={() => toggleArtist(artist)} />
          </Badge>
        ))}
