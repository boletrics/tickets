...
export function EventCarousel({ title, events, showViewAll = true, viewAllHref = "/search" }: EventCarouselProps) {
  ...
  const formatPrice = (price: number) => {
    return locale === "es" ? `$${price.toLocaleString("es-MX")}` : `$${price.toLocaleString("en-US")}`
  }
  ...
  return (
    <section className="py-6">
      ...
      >
        {events.map((event) => (
          <Link key={event.id} href={`/events/${event.id}`} className="flex-shrink-0 w-[280px] md:w-[300px] snap-start">
