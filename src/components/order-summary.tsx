...
export function OrderSummary({ event }: OrderSummaryProps) {
  ...
  const summary = useMemo(() => {
    const tickets = Object.entries(selectedTickets || {})
      ...
      .filter(Boolean) as Array<{ ticket: any; quantity: number; subtotal: number }>
    ...
    <Card className="sticky top-24">
      ...
      <CardContent className="space-y-4">
        {summary.totalTickets === 0 ? (
          <p className="text-sm text-muted-foreground text-center py-8">{t("tickets.selectTickets")}</p>
        ) : (
          <>
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">{t("tickets.subtotal")}</span>
                <span className="font-medium">{formatPrice(summary.subtotal)}</span>
              </div>

              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">{t("tickets.fees")}</span>
