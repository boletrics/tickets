...
export default function SearchPage() {
	...
	const filteredEvents = useMemo(() => {
		...
		if (filters.artists.length > 0) {
			filtered = filtered.filter(
				(event) => event.artist && filters.artists.includes(event.artist),
			);
		}
							...
							<Badge key={v} variant="secondary" className="gap-1">
								...
								<X
									...
									onClick={() =>
										setFilters({
											...filters,
											venues: filters.venues.filter((x) => x !== v),
										})
									}
								/>
							</Badge>
												...
												<CardContent className="flex-1 p-3 md:p-4 flex flex-col justify-between">
													<div>
														<div className="flex items-start justify-between gap-2 mb-1">
															...
															<Badge
																className={`shrink-0 text-xs ${getCategoryColor(event.category)}`}
															>
																{event.category.charAt(0).toUpperCase() +
																	event.category.slice(1)}
															</Badge>
													...
													<div className="flex items-center justify-between mt-2">
														...
														<Button
															variant="ghost"
															size="sm"
															className="gap-1 -mr-2 hidden md:flex"
														>
